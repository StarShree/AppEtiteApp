...
-- Colleges (Short IDs: STAN, IMP, SIT)
INSERT INTO colleges (college_id, name, location) VALUES
...
-- Canteens (2 Canteens per College, using short college codes STAN, IMP, SIT)
INSERT INTO canteens (id, college_id, name, location, operating_hours) VALUES
('canteen_1_1', 'STAN', 'Byte Bites Cyber Cafe', 'Ground Floor, Engineering Wing', '8:00 AM - 9:00 PM'),
('canteen_1_2', 'STAN', 'The Oval Bistro & Express', 'Central Quadrangle, Student Center', '7:30 AM - 10:00 PM'),
('canteen_2_1', 'IMP', 'Imperial North Food Court', 'North Campus, Block A Ground Floor', '8:00 AM - 8:30 PM'),
('canteen_2_2', 'IMP', 'Royal Roast & Juice Lounge', 'East Wing, Science Complex', '8:30 AM - 9:30 PM'),
('canteen_3_1', 'SIT', 'Central Plaza Food Pavilion', 'South Campus, Central Plaza Level 1', '7:30 AM - 9:00 PM'),
('canteen_3_2', 'SIT', 'Tech Hub Cafe & Grille', 'Innovation & IT Tower, Level 2', '8:00 AM - 10:30 PM')
ON DUPLICATE KEY UPDATE college_id=VALUES(college_id), name=VALUES(name), location=VALUES(location);

-- 1. SEED CAMPUS USERS (Separate Table: campus_users)
INSERT INTO campus_users (user_id, id, name, email, password_hash, phone_number, college_id, college_id_str, assigned_canteen_id, campus_id_number, wallet_balance, status) VALUES
(1, 'user_student_1', 'Alex Rivera', 'alex.rivera@campus.edu', 'Student@123', '+1 555-0199', 1, 'STAN', 'canteen_1_1', '2024-CS-042', 350.00, 'ACTIVE'),
(2, 'user_student_2', 'Alex Johnson', 'alex@student.imperial.edu', 'Student@123', '+1 555-0144', 2, 'IMP', 'canteen_2_1', '2023-EE-118', 280.00, 'ACTIVE'),
(3, 'user_student_3', 'Emily Watson', 'emily@student.stjude.edu', 'Student@123', '+1 555-0177', 3, 'SIT', 'canteen_3_1', '2024-ME-077', 310.00, 'ACTIVE')
ON DUPLICATE KEY UPDATE college_id=VALUES(college_id), college_id_str=VALUES(college_id_str);

-- 2. SEED CANTEEN STAFF (Separate Table: canteen_staff)
INSERT INTO canteen_staff (staff_id, id, name, email, password_hash, phone_number, college_id, college_id_str, assigned_canteen_id, designation, status) VALUES
(1, 'user_vendor_1', 'Chef Roberto', 'chef.roberto@bytebites.edu', 'Kitchen@123', '+1 555-0201', 1, 'STAN', 'canteen_1_1', 'Head Chef & Kitchen Operator', 'ACTIVE'),
(2, 'user_vendor_2', 'Chef Mario Rossi', 'kitchen.imperial@appetite.com', 'Kitchen@123', '+1 555-0202', 2, 'IMP', 'canteen_2_1', 'Executive Canteen Chef', 'ACTIVE'),
(3, 'user_vendor_3', 'Chef Gordon Patel', 'kitchen.stjude@appetite.com', 'Kitchen@123', '+1 555-0203', 3, 'SIT', 'canteen_3_1', 'Lead Line Cook & Prep Manager', 'ACTIVE')
ON DUPLICATE KEY UPDATE college_id=VALUES(college_id), college_id_str=VALUES(college_id_str), assigned_canteen_id=VALUES(assigned_canteen_id);

-- 3. SEED COLLEGE ADMINS (Separate Table: college_admins)
INSERT INTO college_admins (admin_id, id, name, email, password_hash, phone_number, college_id, college_id_str, department, status) VALUES
(1, 'user_admin_1', 'Dean Harrison', 'dean.harrison@stanford.edu', 'Admin@123', '+1 555-0301', 1, 'STAN', 'Dean of Student Dining Services', 'ACTIVE'),
(2, 'user_admin_2', 'Prof. Robert Davis', 'admin.imperial@appetite.com', 'Admin@123', '+1 555-0302', 2, 'IMP', 'Canteen Oversight Committee', 'ACTIVE'),
(3, 'user_admin_3', 'Dr. Sarah Connor', 'admin.stjude@appetite.com', 'Admin@123', '+1 555-0303', 3, 'SIT', 'Director of Campus Facilities', 'ACTIVE')
ON DUPLICATE KEY UPDATE college_id=VALUES(college_id), college_id_str=VALUES(college_id_str);
