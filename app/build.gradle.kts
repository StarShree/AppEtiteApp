...
val EMBEDDED_DEBUG_KEYSTORE_BASE64 =
...
fun restoreDebugKeystoreFile() {
  val debugKeystoreFile = file("${rootDir}/debug.keystore")
  if (!debugKeystoreFile.exists()) {
    try {
      val debugKeystoreBase64File = file("${rootDir}/debug.keystore.base64")
      val cleanBase64 = if (debugKeystoreBase64File.exists()) {
        debugKeystoreBase64File.readText().replace("\\s+".toRegex(), "")
      } else {
        EMBEDDED_DEBUG_KEYSTORE_BASE64
      }
...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.appetite.canteen"
    minSdk = 24
    targetSdk = 36
    versionCode = 1
    versionName = "1.0"
